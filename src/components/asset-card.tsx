import type { AssetDTO } from "@/lib/types";
import { formatEuro, formatPercent } from "@/lib/format";
import { Link } from "@tanstack/react-router";
import { BadgeCheck } from "lucide-react";

interface AssetCardProps {
  asset: AssetDTO;
}

export function AssetCard({ asset }: AssetCardProps) {
  const trustScore = asset.passport?.trustScore ?? asset.health;
  const raisePct = asset.property
    ? Math.min(100, Math.round(asset.property.tokensSoldBps / 100))
    : null;

  return (
    <Link
      to="/assets/$assetId"
      params={{ assetId: asset.slug }}
      className="group overflow-hidden rounded-2xl border border-border bg-surface transition-all hover:border-border-strong"
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-black/40">
        <img
          src={asset.imageUrl}
          alt={asset.name}
          loading="lazy"
          className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.02]"
        />
        <div className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-t from-black/70 to-transparent" />
        {asset.property?.cultureSegment && (
          <span className="absolute left-3 top-3 rounded-full border border-white/20 bg-black/40 px-2.5 py-0.5 text-[10px] font-medium capitalize text-white backdrop-blur">
            {asset.property.cultureSegment}
          </span>
        )}
        {asset.property?.tokenSymbol && (
          <span className="absolute bottom-3 left-3 rounded-full border border-accent/40 bg-black/50 px-2.5 py-0.5 font-mono text-[10px] text-accent backdrop-blur">
            {asset.property.tokenSymbol}
          </span>
        )}
        {asset.status === "VERIFIED" && (
          <span className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-black/40 px-2 py-0.5 text-[10px] text-white backdrop-blur">
            <BadgeCheck className="h-3 w-3 text-verified" />
            Verified
          </span>
        )}
      </div>
      <div className="p-6">
        <h3 className="text-lg font-semibold tracking-tight">{asset.name}</h3>
        <p className="mt-0.5 text-xs text-muted-foreground">{asset.location}</p>

        {raisePct != null && (
          <div className="mt-4">
            <div className="flex justify-between text-[10px] text-muted-foreground">
              <span>Community raise</span>
              <span className="tabular font-mono">{raisePct}%</span>
            </div>
            <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-border">
              <div className="h-full rounded-full bg-accent" style={{ width: `${raisePct}%` }} />
            </div>
          </div>
        )}

        <div className="mt-5 grid grid-cols-3 gap-3 border-t border-border pt-4">
          <div>
            <p className="eyebrow !text-[9px]">Value</p>
            <p className="tabular mt-1 font-mono text-sm">{formatEuro(asset.valueCents)}</p>
          </div>
          <div>
            <p className="eyebrow !text-[9px]">Yield</p>
            <p className="tabular mt-1 font-mono text-sm text-accent">
              {formatPercent(asset.yieldBps)}
            </p>
          </div>
          <div>
            <p className="eyebrow !text-[9px]">Trust</p>
            <p className="tabular mt-1 font-mono text-sm text-verified">{trustScore}%</p>
          </div>
        </div>
      </div>
    </Link>
  );
}
